 ...
 * have the same type.
 * 
 * Return the reformatted string or return an empty string if it is impossible to reformat 
 * the string.
 * 
 * Example 1:
 * 
...
public class Reformat {
	public String reformat(String s) {
		...
		int startDigits = 0, startLetters = 0;
		...
		if(digits.length() > letters.length()) {
			rs.append(digits.charAt(startDigits++));
		} else {
			rs.append(letters.charAt(startLetters++));
		}
		while(startDigits < digits.length() || startLetters < letters.length()) {
			if(Character.isDigit(rs.charAt(rs.length() - 1))) {
				rs.append(letters.charAt(startLetters++));
			} else {
				rs.append(digits.charAt(startDigits++));
			}
