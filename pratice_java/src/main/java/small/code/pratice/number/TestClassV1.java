...
class TestClassV1 {
    ...
    private static long solved(String chars, int K){
    	...
    	int bIndex = findB(digits, counts, len, map);
    	int currentIndex = bIndex;
    	int nextIndex = nextNextCycle(digits, len, currentIndex);
    	
    	while(nextIndex != bIndex && k < K) {
    		cycle += countCycle(counts, len, currentIndex, nextIndex);
    		if(compare(digits, counts, len, bIndex, nextIndex) == 0) {
    			map.add(cycle);
    			sum += cycle;
    			cycle = 0;
    			k++;
    		...
    		nextIndex = nextNextCycle(digits, len, currentIndex);
    	}
    	if(nextIndex == bIndex) {
    		cycle += countCycle(counts, len, currentIndex, nextIndex);
			map.add(cycle);
